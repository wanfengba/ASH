...
function _getPost($self)
{
    ...
    $sticky_text = Helper::options()->AIndexSticky;
    if ($sticky_text && $page == 1) {
        ...
        foreach ($sticky_arr as $cid) {
            ...
            if ($item->next()) {
                $result[] = array(
                    "article_type" => $item->fields->article_type ? $item->fields->article_type : 'default',
...
function _pushRecord($self)
{
    ...
    $token = Helper::options()->ABaiduToken;
...
function _getMaccmsList($self)
{
    ...
    $cms_api = Helper::options()->AMaccmsAPI;
...
function _getServerStatus($self)
{
    ...
    $api_panel = Helper::options()->ABTPanel;
    $api_sk = Helper::options()->ABTKey;
