...
{
  if ($type) echo Helper::options()->ALazyload;
  else return Helper::options()->ALazyload;
}
...
{
  $gravatarsUrl = Helper::options()->ACustomAvatarSource ? Helper::options()->ACustomAvatarSource : 'https://gravatar.helingqi.com/wavatar/';
...
{
  $JMottoRandom = explode("\r\n", Helper::options()->Aside_Author_Motto);
...
function _getThumbnails($item)
{
  ...
  if (sizeof($result) < 3) {
    $custom_thumbnail = Helper::options()->AThumbnail;
    /* 将for循环放里面，减少一次if判断 */
    if ($custom_thumbnail) {
...
{
  if (Helper::options()->Aside_Author_Nav && Helper::options()->Aside_Author_Nav !== "off") {
    $limit = Helper::options()->Aside_Author_Nav;
    $db = Typecho_Db::get();
    $prefix = $db->getPrefix();
