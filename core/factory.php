...
class Intercept
{
  public static function message($comment)
  {
    ...
    if (preg_match('/\{!\{(.*)\}!\}/', $comment['text'], $matches)) {
      ...
      /* 判断用户输入是否大于字符 */
      if (Helper::options()->ATextLimit && strlen($comment['text']) > Helper::options()->ATextLimit) {
        $comment['status'] = 'waiting';
      } else {
        /* 判断评论内容是否包含敏感词 */
        if (Helper::options()->ASensitiveWords) {
          if (_checkSensitiveWords(Helper::options()->ASensitiveWords, $comment['text'])) {
            $comment['status'] = 'waiting';
          }
        }
        /* 判断评论是否至少包含一个中文 */
        if (Helper::options()->ALimitOneChinese === "on") {
...
/* 邮件通知 */
if (
  Helper::options()->ACommentMail === 'on' &&
  Helper::options()->ACommentMailHost &&
  Helper::options()->ACommentMailPort &&
  Helper::options()->ACommentMailFromName &&
  Helper::options()->ACommentMailAccount &&
  Helper::options()->ACommentMailPassword &&
  Helper::options()->ACommentSMTPSecure
) {
...
class Email
{
  public static function send($comment)
  {
    ...
    $mail->SMTPAuth = true;
    $mail->CharSet = 'UTF-8';
    $mail->SMTPSecure = Helper::options()->ACommentSMTPSecure;
    $mail->Host = Helper::options()->ACommentMailHost;
    $mail->Port = Helper::options()->ACommentMailPort;
    $mail->FromName = Helper::options()->ACommentMailFromName;
    $mail->Username = Helper::options()->ACommentMailAccount;
    $mail->From = Helper::options()->ACommentMailAccount;
    $mail->Password = Helper::options()->ACommentMailPassword;
    $mail->isHTML(true);
    $text = $comment->text;
...
?>
    ...
    <script>
      window.JoeConfig = {
        ...
        playerAPI: '<?php Helper::options()->ACustomPlayer ? Helper::options()->ACustomPlayer() : Helper::options()->themeUrl('library/player.php?url=') ?>',
    ...
    <script src="<?php _getAssets('assets/typecho/config/js/editor.js') ?>"></script>
<?php
  }
}
