<!--风格二-->
    <!--搜索到内容-->
          <?php if ($this->have()) : ?>
            <ul class="joe_archive__list joe_list" data-wow="<?php $this->options->AList_Animate() ?>">
                <div class="Two-Index">
              <?php while ($this->next()) : ?>
                <?php if ($this->fields->mode === "default" || !$this->fields->mode) : ?>
                    <div class="ASH_cat-index wow default essay">
                <div class="ASH_cat-index_item-content">
                    <div class="ASH_cat-index_item-cover-shadow"></div>
                    <div class="ASH_cat-index_item-cover">
                        <img width="100%" height="100%" class="article-cover ls-is-cached lazyload" src="<?php _getLazyload() ?>" data-src="<?php echo _getThumbnails($this)[0] ?>" alt="<?php $this->title() ?>" />
                    </div>
                    <div class="ASH_cat-index_item-info">
                        <a href="<?php $this->permalink() ?>" class="article-title" title="<?php $this->title() ?>" target="_blank" rel="noopener noreferrer">
                            <div class="article-title-link"><?php $this->title() ?></div>
                        </a>
                        <div class="ASH_cat-index_item-meta">               
                            <div class="article-meta-wrap">
                                <span class="post-meta-date"><span class="article-meta-label">发表于<?php $this->date('Y年m月d日'); ?></span>
                            </span></div>
                        </div>
                    </div>
                    <a class="article-content" href="<?php $this->permalink() ?>" title="文章摘要" target="_blank" rel="noopener noreferrer">
                        <div class="article-content-text"><?php _getAbstract($this) ?></div>
                    </a>
                    <div class="ASH_cat-index_item-arrow"></div>
                </div>
            </div>
