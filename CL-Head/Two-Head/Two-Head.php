...
<header class="ASH_Two joe_header <?php echo $this->is('post') ? 'current' : '' ?>">
  <div class="joe_header__above">
    <div class="joe_container">
      ...
      <a title="<?php $this->options->title(); ?>" class="joe_header__above-logo" href="<?php $this->options->siteUrl(); ?>">
        <?php if ($this->options->Anav_Author) : ?>
        <div class="ASH_header__above-logo">
            <div class="title"><?php $this->options->Anav_Author() ?></div>
            <i></i>
        </div>
        <?php endif; ?>
      ...
      <nav class="joe_header__above-nav">
        <a class="item <?php echo $this->is('index') ? 'active' : '' ?>" href="<?php $this->options->siteUrl(); ?>" title="首页">首页</a>
            ...
            <div class="joe_header__below">
              <?php if ($this->is('post')) :  ?>
                ...
                <?php else : ?>
                <div class="joe_header__below-title"><?php $this->options->description() ?></div>
              <?php endif; ?>
        ...
        <?php if (count($pages->stack) <= $this->options->ANavMaxNum) : ?>
          ...
          <?php endforeach; ?>
        <?php else : ?>
          <?php foreach (array_slice($pages->stack, 0, $this->options->ANavMaxNum) as $item) : ?>
            <a class="item <?php echo $this->is('page', $item['slug']) ? 'active' : '' ?>" href="<?php echo $item['permalink'] ?>" title="<?php echo $item['title'] ?>"><?php echo $item['title'] ?></a>
          <?php endforeach; ?>
          <div class="joe_dropdown" trigger="hover" placement="60px" style="margin-right: 15px;">
            ...
            <nav class="joe_dropdown__menu">
              <?php foreach (array_slice($pages->stack, $this->options->ANavMaxNum) as $item) : ?>
        ...
        <?php
        $custom = [];
        $custom_text = $this->options->ACustomNavs;
        if ($custom_text) {
          $custom_arr = explode("\r\n", $custom_text);
  ...
  <div class="joe_header__slideout">
    <img width="100%" height="150" class="joe_header__slideout-image" src="<?php $this->options->Aside_Wap_Image() ?>" alt="侧边栏壁纸" />
    <div class="joe_header__slideout-author">
      <img width="50" height="50" class="avatar lazyload" src="<?php _getAvatarLazyload(); ?>" data-src="<?php $this->options->Aside_Author_Avatar ? $this->options->Aside_Author_Avatar() : _getAvatarByMail($this->authorId ? $this->author->mail : $this->user->mail) ?>" alt="博主昵称" />
      <div class="info">
        <a class="link" href="<?php $this->options->JAside_Author_Link() ?>" target="_blank" rel="noopener noreferrer nofollow"><?php $this->options->Aside_Author_Nick ? $this->options->Aside_Author_Nick() : ($this->authorId ? $this->author->screenName() : $this->user->screenName()); ?></a>
